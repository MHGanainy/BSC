...
class BoundingBox:
    ...
    def __str__(self):
        return 'width: {}, height: {}, left: {}, top: {}'.format(self._width,
                self._height, self._left, self._top)
                ...
                if rs['Type'] == 'CHILD':
                    for cid in rs['Ids']:
                        if blockMap[cid]['BlockType'] == 'WORD':
                            self._words.append(Word(blockMap[cid],
                                    blockMap))
...
==========
'''
        s = str(s) + str(self._text) + '\n'
        s = str(s) + '''Words
----------
'''
        for word in self._words:
            s = str(str(s) + '[{}]'.format(str(word)))
        return str(s)
                ...
                if rs['Type'] == 'CHILD':
                    for cid in rs['Ids']:
                        ...
                        if blockType == 'WORD':
                            ...
                            self._content.append(w)
                            self._text = str(self._text) + str(w.text) + ' '
                        elif blockType == 'SELECTION_ELEMENT':
                            se = SelectionElement(blockMap[cid],
                                    blockMap)
                            self._content.append(se)
...
'''
    ...
    def _parse(self, blockMap):
        for item in self._blocks:
            if item['BlockType'] == 'PAGE':
                ...
                self._text = str(self._text) + str(l.text) + '\n'
    ...
    def getTextWithAccount(self):
        wordswithAccount = []
        for item in self._lines:
            if "Account" in item._text:
                for word in item._words:
                    if "Account" in word._text:
                        wordswithAccount.append(word)
        return wordswithAccount
    
    def getTextWithOnlyNum(self):
        wordswithOnlyNum = []
        for item in self._lines:
            if item._text.isdecimal():
                wordswithOnlyNum.append(item._words[0])
        return wordswithOnlyNum
...
'''
    ...
    def _parse(self):
        ...
        (self._responseDocumentPages, self._blockMap) = \
